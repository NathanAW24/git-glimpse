...
def iterative_refinement_summarization(model, text, chunk_size=4000):
    ...
    print("Generating initial summary for iterative refinement...")
    current_summary = summarize_with_gpt(model, chunks[0])

    # Template for refinement
    refine_system_prompt = "You are a helpful assistant specializing in iterative refinement of summaries. Your summary need to be detailed as this is for coding. If there is code, make sure you include it. Your summary must be long and detailed"
    refine_user_template = """Existing summary so far:
{existing_summary}

New context:
{new_context}

Refine the summary to incorporate the new context, keeping it concise and coherent.
Your summary need to be detailed.  If there is code, make sure you include it. 
"""

    # Refine with each subsequent chunk
    for i, chunk in enumerate(chunks[1:], start=2):
        print(f"Refining summary with chunk {i}/{len(chunks)}...")
        user_prompt = refine_user_template.format(
            existing_summary=current_summary, new_context=chunk)
        current_summary = call_llm(
            refine_system_prompt, user_prompt, model=model, temperature=0.7).strip()
...
def main():
    ...
    # Summarize and evaluate each model with existing method
    ...
    #     pprint({f"{model}_two_step_results": model_results})

    # Summarize and evaluate each model with iterative refinement (ADDED CODE)
    for model in gpt_models:
        print(f"\nSummarizing with {model} using iterative refinement...")
        model_iterative_results = []
        for i, doc in enumerate(documents):
            iterative_summary = iterative_refinement_summarization(model, doc)
            iterative_score = evaluate_summaries_harimplus(
                [doc], [iterative_summary])
            model_iterative_results.append({
                "document_id": i + 1,
                "original_text": doc,
                "iterative_summary": iterative_summary,
                "iterative_score": iterative_score
            })
        # Add iterative results to the main results dictionary
        results[f"{model}_iterative"] = model_iterative_results
        pprint({f"{model}_iterative_results": model_iterative_results})
